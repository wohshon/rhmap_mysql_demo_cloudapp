...
function helloRoute() {
  ...
  hello.get('/', function(req, res) {
   ...
   fh.service({
      ...
      var strResponse='No matching records: '+world;
      if (response.rows) {
        strResponse='';
        console.log(Object.keys(response)+'----------------------');
        for (var i=0; i < response.rows.length;i++) {
            strResponse+='<div>id : '+response.rows[i].id+'</div>';
            strResponse+='<div>name : '+response.rows[i].name+'</div>';
            strResponse+='<div>email : '+response.rows[i].email+'</div>';
        }
